...
async def main():
    ...
    ap.add_argument("--silence-mode", default="stream", choices=["stream", "end", "stop"])
    ...
    client = genai.Client()
    log("connect", model=a.model, thinking=a.thinking, silence_mode=a.silence_mode)
    async with client.aio.live.connect(model=a.model, config=cfg) as session:
        ...
        # --silence-mode: "stream" keeps sending zero frames; "end" mimics OpenClaw's bridge
        # (zeros for 500 ms, then audioStreamEnd, then nothing until speech); "stop" stops
        # sending after 500 ms of zeros without audioStreamEnd.
        async def mic_sender():
            silence = bytes(CHUNK_BYTES)
            silent_ms, ended = 0, False
            while not done.is_set():
                try:
                    ...
                    chunk = None
                if chunk is None:
                    if a.silence_mode != "stream" and silent_ms >= 500:
                        if a.silence_mode == "end" and not ended:
                            log("send_audio_stream_end")
