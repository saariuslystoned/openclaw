...
import {
  buildThinkingConfig,
  isGemini38LiveExtendedThinkingModel,
  isGemini38LiveModel,
...
describe("Gemini 3.8 Live model contracts", () => {
  it("classifies the 3.8 Live model ids", () => {
    expect(isGemini38LiveModel("gemini-3.8-live")).toBe(true);
    expect(isGemini38LiveModel("models/gemini-3.8-live-extended-thinking")).toBe(true);
    expect(isGemini38LiveExtendedThinkingModel("gemini-3.8-live")).toBe(false);
    expect(isGemini38LiveExtendedThinkingModel("gemini-3.8-live-extended-thinking")).toBe(true);
    expect(isGemini38LiveModel("gemini-3.1-flash-live-preview")).toBe(false);
  });
...
describe("buildGoogleRealtimeVoiceProvider with Gemini 3.8 Live", () => {
  ...
  it("interrupts Gemini 3.8 Live Extended Thinking output with an empty client turn on barge-in", async () => {
    ...
    expect(session.sendClientContent).toHaveBeenCalledWith({ turnComplete: true });
  });
