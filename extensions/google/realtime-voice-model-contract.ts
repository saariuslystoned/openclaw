...
export function isGemini38LiveModel(model: string): boolean {
  return normalizeGoogleLiveModelId(model).startsWith("gemini-3.8-live");
}

export function isGemini38LiveExtendedThinkingModel(model: string): boolean {
  const modelId = normalizeGoogleLiveModelId(model);
...
// Gemini Live has no response.cancel. Extended Thinking documents that a client-content
// turn with turnComplete interrupts active generation; the server then emits
// `interrupted` and `turnComplete`. Other Live models only interrupt through server-side
// VAD on input audio.
